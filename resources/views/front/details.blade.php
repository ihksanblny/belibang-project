    ...
    <section id="DetailsContent" class="container max-w-[1130px] mx-auto mb-[32px] relative -top-[70px]">
        <div class="flex flex-col gap-8">
            <div class="w-[1130px] h-[700px] flex shrink-0 rounded-[20px] overflow-hidden">
                <img src="{{Storage::url($product->cover)}}" class="w-full h-full object-cover" alt="{{$product->name}}">
            </div>
                ...
                <div
                        ...
                        <div class="flex items-center gap-[10px] mt-1">
                            @foreach($product->logos as $logo)
                                <a href=""
                                    ...
                                    <img src="{{ asset('storage/' . $logo->logo_path) }}" class='p-[5px]' alt="Product Logo">
                                </a>
                            @endforeach
                        </div>
                    </div>
                    <div class="flex flex-row flex-wrap gap-4 items-center mt-4">
                        @foreach($product->tags as $tag) 
                            <a href=""
                                class="tags p-[4px_8px] border border-[#414141] rounded-[4px] h-fit w-fit text-xs text-belibang-light-grey hover:bg-[#2A2A2A] transition-all duration-300">
                                {{ $tag->name }}
                            </a>
                        @endforeach
                    </div>
            ...
            <div class="product-card flex flex-col rounded-[18px] bg-[#181818] overflow-hidden">
                ...
                <div class="p-[10px_14px_12px] h-full flex flex-col justify-between gap-[14px]">
                    ...
                    <div class="flex items-center gap-[6px]">
                        ...
                        <a href="{{route('front.details', $product->slug)}}" class="font-semibold text-xs text-belibang-grey">{{$product->creator->name}}</a>
                    </div>
                </div>
            </div>
            @empty
